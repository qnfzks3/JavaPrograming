    ...
    public static void main(String[] args) {
        ...
        for (EMPVO semp : empdata) {
            System.out.printf(fmt, emp.getEmpno(), emp.getFname(),
                    semp.getEmail(), emp.getJobid(), emp.getDeptno());
        }

        //사원 상세조회
       System.out.println("조회할 사원번호는?");
        int sempno = sc.nextInt();
        EMPVO semp = EMPDAOImpl.selectOneEmp(sempno);
        if(emp != null) System.out.println(sempno);
...
class EMPDAOImpl{  //5개 작성해야한다  insert select 전체 , select 검색, update 수정  , delete 삭제
    ...
    public static int insertEmp(EMPVO emp){ //emp는 EMPVO로 정의한 변수들이 모여있는 함수 - 배열과 같이 쓰임
        //inser 5. insert연결  - 1) 연결 ,2)prepare~로 SQL문을 pstmt에 사용해준다. 3)get으로 각 칼럼 가져오기 4)
        Connection conn = null;
        PreparedStatement pstmt = null;
        int cnt= 0;

        try {
            conn=JDBCUtilTest.testConn();
            ...
            JDBCUtilTest.closeConn(null,pstmt,conn);
    ...
    public static List<EMPVO> selectEmp(){
        ...
        try {
            ...
            JDBCUtilTest.closeConn(rs, pstmt, conn);
        }
